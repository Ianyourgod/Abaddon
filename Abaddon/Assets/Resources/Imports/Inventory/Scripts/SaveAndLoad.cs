...
public class SaveAndLoad : MonoBehaviour
{
	...
	[Tooltip("Select the time period in seconds between multiple saves. It is recommended to use something bigger (eg. 2 minutes) but not necessary.")]
	...
	private void Start()
	{
		...
		if (autoLoad) StartCoroutine(AutoLoad(0.2f));
	}

	private void Update()
	{
		if (Input.GetKeyDown(SaveKey)) Save();
		else if (Input.GetKeyDown(LoadKey)) Load();

		if (autoSave && !autoSaved) StartCoroutine(AutoSave(timeBetweenSaves));
	}
	...
	void Save()
	{
		...
		{
			if (inv.slots[i].slotsItem) items++;
		}
		List<Slot> allSlots = new List<Slot>();

		foreach (Slot slot in inv.slots) allSlots.Add(slot);
		foreach (Slot slot in inv.equipSlots) allSlots.Add(slot);
		...
		else print("There are no items in your inventory!");
	}
	...
	void Load()
	{
		...
		if (itemsToLoad != null)
		{
			if (itemsToLoad.Count > 0)
			{
				...
				for (int i = itemsToLoad[0].slotIndex; i < readySlots.Length; i++)
				{
					foreach (ItemLoad z in itemsToLoad)
					{
						if (i == z.slotIndex)
						{
							Item b = Instantiate(items[z.id - 1], allSlots[i].transform).GetComponent<Item>();
			...
			else print("There aren't any save files found!");
		}
		else print("There aren't any save files found!");
	}

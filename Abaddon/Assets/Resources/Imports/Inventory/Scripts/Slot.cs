...
public class Slot : MonoBehaviour
{
    [Header("Item")]
    [Tooltip(
        "This is the current item that is inside the slot. If there is not an item it this box this means your slot is currently empty."
    )]
    [ReadOnly]
    ...
    private void Update()
    {
        ...
        if (vText && !beingDragged)
        {
            ...
            vText.GetComponent<RectTransform>().offsetMax = new Vector2(
                -5,
                vText.GetComponent<RectTransform>().offsetMax.y
            ); // why is it like this? i have no idea.
            vText.GetComponent<RectTransform>().offsetMax = new Vector2(
                vText.GetComponent<RectTransform>().offsetMax.x,
                -0
            ); // it doesn't seem like there should be a reason for it to be on two lines
        ...
        if (!GetComponent<EquipmentSlot>() && !GetComponent<CraftingSlot>())
        {
            if (!beingDragged)
            {
                if (amountText)
                {
                    if (slotsItem)
                    {
                        ...
                        {
                            if (slotsItem && slotsItem.amountInStack > 1)
                                StartCoroutine(Wait(0.0001f));
                ...
                if (itemImage)
                {
                    ...
                    {
                        if (slotsItem.amountInStack > 0)
                            itemImage.sprite = slotsItem.itemSprite;
                        else
                        {
                            Destroy(slotsItem.gameObject);
            ...
            else if (beingDragged)
            {
                ...
                if (!textChecked && transform.GetChild(0).transform.GetChild(0))
                {
                    if (slotsItem.amountInStack > 1)
                    {
                        ...
                        clone.GetComponent<RectTransform>().offsetMax = new Vector2(
                            -0,
                            clone.GetComponent<RectTransform>().offsetMax.y
                        );
                        clone.GetComponent<RectTransform>().offsetMax = new Vector2(
                            clone.GetComponent<RectTransform>().offsetMax.x,
                            -0
                        );
            ...
            {
                if ((slotsItem.amountInStack - Mathf.RoundToInt(slotsItem.amountInStack / 2)) > 1)
                    amountText.text = (
                        slotsItem.amountInStack - Mathf.RoundToInt(slotsItem.amountInStack / 2)
                    ).ToString();
                else
                    amountText.text = "";
                ...
                if (!textChecked && transform.GetChild(0).transform.GetChild(0))
                {
                    ...
                    {
                        clone = Instantiate(amountText.gameObject);
                        clone.GetComponent<TMP_Text>().text = Mathf
                            .RoundToInt(slotsItem.amountInStack / 2)
                            .ToString();
                        clone.transform.SetParent(drag.followMouseImage.transform);
                        clone.GetComponent<RectTransform>().anchoredPosition = new Vector2(0, 0);
                        clone.GetComponent<RectTransform>().anchorMin = new Vector2(0, 0);
            ...
            if (transform.childCount <= 1 && itemImage)
                itemImage.sprite = defaultSprite;
        }
        ...
        if (slotsItem)
        {
            if (!removeAll)
            {
                for (int i = 0; i < removeQuantity; i++)
                {
                    if (slotsItem.amountInStack > 0)
                    {
                        thrownItem = Instantiate(
                            slotsItem.gameObject,
                            player.GetComponent<Inventory>().throwPosition.transform.position,
                            Quaternion.identity
                        );
    ...
    IEnumerator Wait(float time)
    {
        ...
        if (slotsItem && slotsItem.amountInStack >= 2)
            amountText.gameObject.SetActive(true);
    }

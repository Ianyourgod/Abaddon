...
public class DragAndDrop : MonoBehaviour
{
    ...
    [HideInInspector]
    public List<Item> itemAmountsDescending,
        itemAmountsAscending,
        positiveItems;
    ...
    private void Start()
    {
        ...
        followMouseImage.color = new Color(
            followMouseImage.color.r,
            followMouseImage.color.g,
            followMouseImage.color.b,
            0
        );
    }
    ...
    private void Update()
    {
        if (followMouseImage.transform.childCount > 1)
            for (int i = 0; i < followMouseImage.transform.childCount; i++)
                if (i < followMouseImage.transform.childCount)
                    Destroy(followMouseImage.transform.GetChild(i));
        ...
        if (invObj.activeSelf)
        {
            ...
            else if (Input.GetMouseButton(0) && !Input.GetMouseButton(1))
            {
                if (curSlot && curSlot.GetComponent<Slot>().slotsItem)
                {
                    ...
                    if (curSlot.transform.childCount > 1)
                        followMouseImage.sprite = curSlot
                            .transform.GetChild(1)
                            .GetComponent<Item>()
                            .itemSprite;
                    if (curSlot)
                        curSlot.GetComponent<Slot>().beingDragged = true;
                    if (curSlot)
            ...
            else if (Input.GetMouseButtonUp(0))
            {
                if (curSlot && curSlot.GetComponent<Slot>().slotsItem)
                {
                    ...
                    if (newObj && newObj != curSlot)
                    {
                        ...
                        if (
                            newObj.GetComponent<EquipmentSlot>()
                            && newObj.GetComponent<EquipmentSlot>().equipmentType
                                != curSlotsItem.equipmentType
                        )
                        {
                            return;
                        }
                        ...
                        if (newObj.GetComponent<Slot>().slotsItem)
                        {
                            ...
                            if (
                                objectsItem.ItemID == curSlotsItem.ItemID
                                && objectsItem.amountInStack + curSlotsItem.amountInStack
                                    <= objectsItem.maxStackSize
                                && !newObj.GetComponent<EquipmentSlot>()
                            )
                            {
                            ...
                            {
                                if (
                                    curSlotsItem.equipmentType == objectsItem.equipmentType
                                    || !curSlot.GetComponent<EquipmentSlot>()
                                )
                                {
            ...
            else if (Input.GetMouseButton(1) && !Input.GetMouseButton(0))
            {
                if (curSlot && curSlot.GetComponent<Slot>().slotsItem)
                {
                    ...
                    {
                        if (curSlot)
                            curSlot.GetComponent<Slot>().beingSplitted = true;
                        followMouseImage.color = new Color(255, 255, 255);
                        if (curSlot.transform.childCount > 1)
                            followMouseImage.sprite = curSlot
                                .transform.GetChild(1)
                ...
                if (curSlot && curSlot.GetComponent<Slot>().slotsItem)
                {
                    ...
                    if (curSlot.transform.childCount > 1)
                        followMouseImage.sprite = curSlot
                            ...
                            .GetComponent<Item>()
                            .itemSprite;
                    if (curSlot && curSlotsItem && curSlotsItem.amountInStack > 1)
                        curSlot.GetComponent<Slot>().beingSplitted = true;
                    if (curSlot)
                        curSlot.GetComponent<Slot>().beingDragged = false;
                }
                    ...
                    if (curSlotsItem.amountInStack > 1)
                    {
                        ...
                        if (newObj && newObj != curSlot)
                        {
                            ...
                            if (newObj.GetComponent<Slot>().slotsItem)
                            {
                                ...
                                if (
                                    objectsItem.ItemID == curSlotsItem.ItemID
                                    && objectsItem.amountInStack != objectsItem.maxStackSize
                                    && !newObj.GetComponent<EquipmentSlot>()
                                )
                                {
                                    ...
                                    if (
                                        item.amountInStack
                                            + Mathf.RoundToInt(curSlotsItem.amountInStack / 2)
                                        > item.maxStackSize
                                    )
                                    {
                                    ...
                                    {
                                        item.amountInStack += Mathf.RoundToInt(
                                            curSlotsItem.amountInStack / 2
                                        );
                                        curSlotsItem.amountInStack -= Mathf.RoundToInt(
                                            curSlotsItem.amountInStack / 2
                                        );
                                    }
                                }
                            ...
                            else
                            {
                                ...
                                clone.amountInStack -= Mathf.RoundToInt(
                                    curSlotsItem.amountInStack / 2
                                );
                                if (oldSlot)
                                    clone.transform.parent = oldSlot.transform;
                                else
                                    clone.transform.parent = curSlot.transform;
                                curSlotsItem.transform.parent = newObj.transform;
                                curSlotsItem.amountInStack = Mathf.RoundToInt(
                                    curSlotsItem.amountInStack / 2
                                );
        ...
        {
            dropItemFromCursor();
            // followMouseImage.GetComponent<Image>().sprite = null;
    ...
    public void dropItemFromCursor()
    {
